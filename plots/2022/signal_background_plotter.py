...
class SignalBackgroundCanvas():
    ...
    def _handle_binning(self):
        print("[INFO] Binning already harmonized upstream. Skipping _handle_binning().")
        return
        ...
        if "rebin" in self.config.keys():
            ...
            for i, (name, hist) in enumerate(self.other_backgrounds):
                try:
                    new_hist = hist.Rebin(len(bins)-1, f"{name}_rebin", bins)
                    new_hist.SetDirectory(0)
                    self.other_backgrounds[i] = (name, new_hist)
                    self._objects_to_keep.append(new_hist)
                    print(f"Successfully variable-rebinned other background: {name}")
                except Exception as e:
                    print(f"Error variable-rebinning other background {name}: {e}")
        else:
            # 리빈하지 않는 경우에도 총 빈수 출력
            if self.signal_hists:
                test_hist = list(self.signal_hists.values())[0]
                print(f"No rebinning requested. Total bins: {test_hist.GetNbinsX()}")
            elif self.top_backgrounds:
                test_hist = self.top_backgrounds[0][1] 
                print(f"No rebinning requested. Total bins: {test_hist.GetNbinsX()}")
    ...
    def _build_background_stack(self):
        ...
        if self.others_hist is not None:
            ...
            self.others_hist.SetFillColor(BACKGROUND_COLORS[3])  # Gray
            self.others_hist.SetLineColor(BACKGROUND_COLORS[3])
    ...
    def draw(self):
        ...
        latex.DrawLatex(0.12, 0.95, "CMS")  # Moved from 0.91 to 0.95
        
        latex.SetTextSize(0.035)
        latex.SetTextFont(52)
        latex.DrawLatex(0.20, 0.95, "Simulation Preliminary")  # Moved from 0.91 to 0.95
        
        latex.SetTextFont(42)
        latex.DrawLatex(0.65, 0.95, "7.9104 fb^{-1} (13.6 TeV)")  # Moved from 0.70, 0.91 to 0.65, 0.95
        ...
        if hasattr(self, 'ratio') and self.ratio:
            ...
            ratio_dummy.GetYaxis().SetTitleSize(0.10)
