#ifndef LRSM_TBChannel_2AK8_h
#define LRSM_TBChannel_2AK8_h

#include "AnalyzerCore.h"
#include "SystematicHelper.h"

class LRSM_TBChannel_2AK8 : public AnalyzerCore {
public:
    LRSM_TBChannel_2AK8();
    ~LRSM_TBChannel_2AK8();
    ...
    RVec<FatJet> RemoveOverlapWithMuons(const RVec<FatJet>& jets, const RVec<Muon>& muons, float deltaR_cut = 0.4);
    RVec<FatJet> RemoveOverlapWithFatJets(const RVec<FatJet>& jets, const RVec<FatJet>& fatjets, float deltaR_cut = 0.8);
    RVec<FatJet> RemoveOverlapWithMuonsFatJet(const RVec<FatJet>& fatjets, const RVec<Muon>& muons, float deltaR_cut = 0.8);
    bool PassEventSelection(const RVec<Muon>& muons, const RVec<FatJet>& bjets, const RVec<FatJet>& topjets);
    bool PassKinematicCuts(const RVec<Muon>& muons);
    bool PassDileptonMassCut(const RVec<Muon>& muons);
    float CalculateWRMass(const RVec<Muon>& muons, const RVec<FatJet>& bjets, const RVec<FatJet>& topjets);
    float CalculateNeutrinoMass(const RVec<Muon>& muons, const RVec<FatJet>& jets, const RVec<FatJet>& topjets);
};
